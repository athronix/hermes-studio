...
describe('desktop updater helpers', () => {
  ...
  it('uses verified process-tree termination for Windows updater cleanup', () => {
    ...
    expect(installerSource).toContain('!macro bypassLegacyHermesStudioUninstaller')
    expect(installerSource).toContain('DeleteRegKey SHELL_CONTEXT "${UNINSTALL_REGISTRY_KEY}"')
    expect(installerSource).toContain('!insertmacro bypassLegacyHermesStudioUninstaller')
