(() => {
  ...
  const setSelectedColor = (color) => {
    ...
    if (meta && statusLabel && statusSwatch) {
      statusLabel.textContent = meta.label;
      statusSwatch.style.background = meta.value;
    }
    if (mobileQuery.matches) updateOrbit(cycleStart, true);
  };
  ...
  const updateOrbit = (time, snapToActive = false) => {
    ...
    const rotation = snapToActive ? -activeIndex * ((Math.PI * 2) / orbitItems.length) : elapsed * Math.PI * 2;
  ...
  if (stage && orbitItems.length) {
    ...
    stage.addEventListener("pointerleave", () => { paused = false; cycleStart = performance.now(); });
